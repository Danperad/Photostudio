...
public static class EntityConfigure
{
    ...
    public static void ServiceDataConfigure(EntityTypeBuilder<Service> builder)
    {
        ...
        services.Add(new Service("Аренда одежды", "Временное изъятие из нашего склада платья для ваших нужд", 5000)
            { ID = 5 });
        services.Add(new Service("Аренда реквизита", "Временное изъятие из нашего склада реквизита для ваших нужд",
            3000) { ID = 6 });
        services.Add(new Service("Аренда помещения", "Временное изъятие из нашего склада помещения для ваших нужд",
            3000) { ID = 7 });
        services.Add(new Service("Печать фотографий", "Распечатаем ваши фотографии", 100) { ID = 8 });
        services.Add(new Service("Детская фотосессия", "Фотографирование детей от одного из фотографов", 3000)
            { ID = 9 });
        services.Add(new Service("Аренда детской одежды", "Временное изъятие из нашего склада платья для ваших нужд",
            5000) { ID = 10 });
        services.Add(new Service("Свадебная фотосессия", "Фотографирование на свадьбе от одного из фотографов", 4500)
            { ID = 11 });
        services.Add(new Service("Свадебная видеосъёмка", "Видеосъёмка на свадьбе от одного из операторов", 10000)
            { ID = 12 });
    ...
    public static void RentedItemDataConfigure(EntityTypeBuilder<RentedItem> builder)
    {
        ...
        items.Add(new RentedItem("Платье", "55 размер", 1, 1000, true, true) { ID = 1 });
        items.Add(new RentedItem("Костюм", "45 размер", 1, 1300, true, false) { ID = 2 });
        items.Add(new RentedItem("Пистолет ММГ", "Копия изместного пистолета", 8, 800, false, false) { ID = 3 });
        items.Add(new RentedItem("Голубь", "Белый, серый, чёрный", 16, 10000, false, false) { ID = 4 });
        items.Add(new RentedItem("Пластмассовые фрукты", "В асортименте", 100, 200, false, false) { ID = 5 });
